-- ============================================================
-- Inovimmo: RLS-Fix + Seed für Admin-Zugriff & Chilenaustrasse
-- Im Supabase Dashboard → SQL Editor ausführen
-- ============================================================

-- ── 0. Fehlende Spalten sicherstellen ─────────────────────────
...
-- ── 1. RLS-POLICES: Admin darf alles sehen ───────────────────

-- Liegenschaften: Admin darf alles
...
-- Wohnungen: Admin darf alles, Verwalter über Liegenschaft, Mieter über Mietverhältnis
...
-- Mietverhältnisse: Admin darf alles
...
-- Mieter: Bug-Fix erstellt_von → verwalter_id
...
-- ── 2. KURT RUSCH PROFIL (falls nicht existent) ───────────────
...
INSERT INTO public.profiles (id, email, full_name, role, firma, strasse, plz, ort)
SELECT u.id, u.email, 'Kurt Rusch', 'verwalter', 'Rusch Immobilien', 'Rebackerstrasse', '8955', 'Oetwil an der Limmat'
FROM auth.users u
...
BEGIN
  ...
  IF EXISTS (SELECT 1 FROM liegenschaften WHERE name = 'Chilenaustrasse 15' AND verwalter_id = v_verwalter_id) THEN
    RAISE NOTICE 'Chilenaustrasse 15 already exists. Skipping seed.';
    RETURN;
  END IF;
  ...
  -- MIETVERHAELTNISSE — Chilenaustrasse 15
  ...
  -- MIETVERHAELTNISSE — Chilenaustrasse 17
