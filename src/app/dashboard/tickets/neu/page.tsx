      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="form-section">
          ...
          <div className="grid grid-cols-2 gap-3">
            <div>
              ...
              <select
                required value={form.liegenschaft_id}
                onChange={(e) => update("liegenschaft_id", e.target.value)}
              ...
              >
                ...
                {liegenschaften.map(l => (
                  <option key={l.id} value={l.id}>{l.name} — {l.ort}</option>
                ))}
              </select>
            </div>
            <div>
              ...
              <select
                value={form.wohnung_id}
                onChange={(e) => update("wohnung_id", e.target.value)}
                disabled={!form.liegenschaft_id}
                className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[hsl(214,76%,49%)] disabled:opacity-40"
              >
        ...
        <div className="form-section">
          ...
          <div className="space-y-3">
            <div>
              ...
              <input
                type="text" required value={form.titel}
                onChange={(e) => update("titel", e.target.value)}
                placeholder="z.B. Heizung ausgefallen Wohnung 3.OG"
                className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[hsl(214,76%,49%)] focus:ring-2 focus:ring-blue-100"
              />
            </div>
            <div>
              ...
              <textarea
                required rows={4} value={form.beschreibung}
                onChange={(e) => update("beschreibung", e.target.value)}
                placeholder="Bitte beschreiben Sie das Problem so detailliert wie möglich…"
                className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[hsl(214,76%,49%)] focus:ring-2 focus:ring-blue-100 resize-none"
              />
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <select
                  ...
                  className="w-full px-3 py-2.5 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[hsl(214,76%,49%)]"
                >
                  {KATEGORIEN.map(k => (
              ...
              <div>
                ...
                <input
                  type="number" min="0" step="50" value={form.budget_max}
        ...
        <div className="form-section">
          ...
          <div className="grid grid-cols-3 gap-2">
            {PRIORITAETEN.map(p => (
              ...
              >
                <input type="radio" name="prio" value={p.value} checked={form.prioritaet === p.value}
                  onChange={(e) => update("prioritaet", e.target.value)} className="sr-only" />
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            className="flex-1 py-2.5 bg-[hsl(214,76%,49%)] text-white font-semibold rounded-xl hover:bg-[hsl(214,76%,44%)] disabled:opacity-50 transition-colors text-sm"
          >
