...
export default function EscrowActions({ escrow }: { escrow: Escrow }) {
  ...
  async function pay() {
    ...
    try {
      ...
      toast.success(`CHF ${Number(escrow.betrag).toLocaleString("de-CH")} einbezahlt.`);
      router.refresh();
  ...
  if (escrow.status === "ausstehend") return <button onClick={pay} disabled={loading} className="px-3 py-1.5 bg-[hsl(214,76%,49%)] text-white text-xs font-semibold rounded-lg disabled:opacity-50">{loading ? "…" : "💳 Einzahlen"}</button>;
