...
export async function approveUser(formData: FormData): Promise<void> {
  ...
  if (!userId) redirect('/dashboard/admin/benutzer?error=Benutzer-ID+fehlt');

  let supabaseAdmin;
  try { supabaseAdmin = getSupabaseAdmin(); } catch (err) {
    const msg = err instanceof Error ? err.message : 'Konfigurationsfehler';
    redirect(`/dashboard/admin/benutzer?error=${encodeURIComponent(msg)}`);
  }
  ...
  if (error) redirect(`/dashboard/admin/benutzer?error=${encodeURIComponent(error.message)}`);
  redirect('/dashboard/admin/benutzer?success=Benutzer+freigeschaltet');
}
...
export async function disableUser(formData: FormData): Promise<void> {
  ...
  if (error) redirect(`/dashboard/admin/benutzer?error=${encodeURIComponent(error.message)}`);
  redirect('/dashboard/admin/benutzer?success=Benutzer+deaktiviert');
}
