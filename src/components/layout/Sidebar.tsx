...
export default function Sidebar({ profile }: { profile: Profile | null }) {
  ...
  const role = profile?.role ?? "verwalter";
    ...
    <aside className="sidebar overflow-y-auto" style={{ width: 240 }}>
      <div className="px-4 py-5 border-b border-white/10 flex-shrink-0">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 bg-[hsl(214,76%,49%)] rounded-lg flex items-center justify-center text-base flex-shrink-0">🏛</div>
          <span className="text-lg font-bold tracking-tight">Inovimmo</span>
        </div>
        <p className="text-white/30 text-xs mt-0.5 ml-10">Swiss Property Intelligence</p>
      </div>
        ...
        {filtered.map((item) => {
          ...
          return (
            <div key={item.href + item.label}>
              {showSection && <p className="text-white/30 text-[10px] font-semibold uppercase tracking-wider px-3 pt-4 pb-1">{item.section}</p>}
      ...
      <div className="px-4 py-3 border-t border-white/10 flex-shrink-0">
        <div className="flex items-center gap-2.5">
          <div className="w-8 h-8 rounded-full bg-[hsl(214,76%,49%)] flex items-center justify-center text-xs font-bold flex-shrink-0">{initials}</div>
          <div className="min-w-0">
            <p className="text-sm font-medium text-white truncate">{profile?.full_name ?? "Benutzer"}</p>
            <p className="text-xs text-white/40 truncate">{profile?.firma ?? role}</p>
          </div>
        </div>
      </div>
    </aside>
